...
struct Buffer {
    ...
    void create(vks::VulkanDevice* vulkanDevice, BufferType bufferType, VkBufferUsageFlags usage, VkDeviceSize size, bool persistentMapped = false, VmaPool pool = VK_NULL_HANDLE)
    {
        if (buffer) destroy();
