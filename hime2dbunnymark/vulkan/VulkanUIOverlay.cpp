...
namespace vks 
{
	...
	void UIOverlay::prepareResources()
	{
        ...
        poolCreateInfo.blockSize = 32 * 1024 * 1024;
        poolCreateInfo.maxBlockCount = 1;

        VkBufferCreateInfo tempBufCreateInfo = { VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO };
        tempBufCreateInfo.size = 1024; // Whatever.
        tempBufCreateInfo.usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT;
        VmaAllocationCreateInfo allocCreateInfo = {};
        allocCreateInfo.usage = VMA_MEMORY_USAGE_CPU_TO_GPU; // Change if needed.
        uint32_t memTypeIndex;
        vmaFindMemoryTypeIndexForBufferInfo(device->allocator, &tempBufCreateInfo, &allocCreateInfo, &memTypeIndex);
        poolCreateInfo.memoryTypeIndex = memTypeIndex;

        vmaCreatePool(device->allocator, &poolCreateInfo, &pool);
	...
	bool UIOverlay::update()
	{
		...
		if ((vertexBuffer.buffer == VK_NULL_HANDLE) || (vertexCount != imDrawData->TotalVtxCount)) {
            ...
            vertexBuffer.create(device, vks::BufferType::transient, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, vertexBufferSize, false, pool);
		...
		if ((indexBuffer.buffer == VK_NULL_HANDLE) || (indexCount < imDrawData->TotalIdxCount)) {
			indexBuffer.destroy();
            indexBuffer.create(device, vks::BufferType::transient, VK_BUFFER_USAGE_INDEX_BUFFER_BIT, indexBufferSize, false, pool);
	...
	void UIOverlay::freeResources()
	{
        ...
        if (pool != VK_NULL_HANDLE) {
            vmaDestroyPool(device->allocator, pool);
        }
